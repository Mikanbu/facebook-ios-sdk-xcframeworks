...
/**
 ...
 @param object the LFSDKAppLink representing the deferred App Link
 @param stop the error during the request, if any
 */
typedef id _Nullable (^ LFSDKInvalidObjectHandler)(id object, BOOL *stop)
NS_SWIFT_NAME(InvalidObjectHandler);

NS_SWIFT_NAME(BasicUtility)
@interface LFSDKBasicUtility : NSObject
...
+ (nullable NSString *)JSONStringForObject:(id)object
                      ...
                      invalidObjectHandler:(nullable LFSDKInvalidObjectHandler)invalidObjectHandler;
